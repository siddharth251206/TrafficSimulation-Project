...
public:
    void add_road(const sf::Vector2f& start_pos, const sf::Vector2f& end_pos);
    void add_double_road(const sf::Vector2f& start_pos, const sf::Vector2f& end_pos, float width, bool need_divider = true);
    ...
    // NEW: Get all junctions for UI interaction
    std::vector<std::shared_ptr<Junction>> get_all_junctions() const;
