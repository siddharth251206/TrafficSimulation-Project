      ...
      ui_font(font)
{
    ...
    // [ADDED - initialize user car colors and named junctions]
...
{
    // [ADDED - named junctions mapping for nicer button labels]
    named_junctions = {
        ...
        if (mbp->button == sf::Mouse::Button::Left)
        {
            ...
            if (selection_mode != SelectionMode::None)
            {
                ...
                if (clicked_junc)
                {
                    ...
                    {
                        case SelectionMode::Car1_SetSource:
                            user_cars[0].source_junction = clicked_junc;
                            user_cars[0].source_name = found_name;
                            user_cars[0].is_spawned = false;
                            break;
                        case SelectionMode::Car1_SetDest:
                            user_cars[0].dest_junction = clicked_junc;
                            user_cars[0].dest_name = found_name;
                            break;
                        case SelectionMode::Car2_SetSource:
                            user_cars[1].source_junction = clicked_junc;
                            user_cars[1].source_name = found_name;
                            user_cars[1].is_spawned = false;
                            break;
                        case SelectionMode::Car2_SetDest:
                            user_cars[1].dest_junction = clicked_junc;
                            user_cars[1].dest_name = found_name;
                            break;
                        case SelectionMode::Car3_SetSource:
                            user_cars[2].source_junction = clicked_junc;
                            user_cars[2].source_name = found_name;
                            user_cars[2].is_spawned = false;
                            break;
                        case SelectionMode::Car3_SetDest:
                            user_cars[2].dest_junction = clicked_junc;
                            user_cars[2].dest_name = found_name;
                            break;
                        default:
                            break;
                    }
                    ...
                    for (int i = 0; i < 3; i++) {
                        if (user_cars[i].source_junction && user_cars[i].dest_junction && !user_cars[i].is_spawned) {
                            const auto& out = user_cars[i].source_junction->get_outgoing_roads();
                            if (!out.empty()) {
                                size_t idx = RNG::instance().getIndex(0, out.size() - 1);
                                if (auto road = out[idx].lock()) {
                                    auto car = std::make_unique<Car>(road, car_texture, user_cars[i].color);
                                    car->set_destination(user_cars[i].dest_junction);  // 🩹 give car a target
                                    road->add(std::move(car));
                                    user_cars[i].is_spawned = true;
                                    user_cars[i].spawned_count++;
                                }
                            }
                        }
                    }
...
void UXController::draw_left_buttons()
{
    for (size_t i = 0; i < left_buttons.size(); ++i)
    {
        ...
        if (car_index >= 0 && car_index < 3) {
            ...
            dot.setPosition({btn.bounds.position.x + btn.bounds.size.x - 14.f,
...
void UXController::draw_highlight_rings()
{
    ...
    // Restore view to default for UI rendering
...
void UXController::render_ui()
{
    ...
    // Draw top bar and left buttons and right HUD in screen space
    draw_top_status_bar();
    draw_left_buttons();
    draw_right_hud();

    // Draw pulsing junction highlight rings in world-space
    draw_highlight_rings();

    // Simple bottom-left instruction text
